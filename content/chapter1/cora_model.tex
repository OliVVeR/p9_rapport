...
The \gls{cora} model takes a set of payloads with some requirements which works as constrains, and other descriptive values, as described in \cref{sec:read_input}, in order to produce a schedule that upholds the specifications.
...
The model is made to optimise in regards to profit, see \cref{sec:read_input}, this is done as the battery level is rarely a concern\cite{gom_space_conversation}.
...
In this query \uppVar{t\_time} it the accumulated time, and \uppVar{ScheduleLength} is the desired length of the schedule.
