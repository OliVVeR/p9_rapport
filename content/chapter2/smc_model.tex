...
During the entire schedule we need to keep track of the battery level, as mentioned in the start of this section we are using \gls{kibam}. In \myref{sec:kibam} we show the equations for calculating the available and bound charge as a function of time. In order to use it in our \gls{smc} model we need to change the equation, to not be a function of time and only give us the difference changes to the available and bound charge each time unit. We are able to do this, because we store the value of available and bound charge as each time unit passes in two clocks, \uppVar{a} and \uppVar{b}. At the start of the model we assign \uppVar{a} to \gls{soc} multiply with the ratio c. likewise we assign \uppVar{b} to \gls{soc} multiply by one minus c.

The equation used to calculate clock \uppVar{a} can be seen in \cref{eq:calcA}. The equation has two purposes, subtract the amount of energy being used by the running payload represented with the variable \uppVar{i}, and add energy to \uppVar{a} from \uppVar{b} based on the difference between the two wells calculated by $k*(b/(1-c)-a/c)$.
\begin{equation}\label{eq:calcA}
\uppInv{a' == -i+k*(b/(1-c)-a/c)} 
\end{equation}

Similar for clock \uppVar{b}, It needs to subtract the amount of energy add to \uppVar{a} and add the amount of charge gained from \uppVar{RechargeRate}. \uppVar{insolation} is a bool that is either one or zero depending on the Insolation template and determines if \uppVar{b} gains charge.
\begin{equation}\label{eq:calcB}
\uppInv{b' == -k*(b/(1-c)-a/c)+(insolation * RechargeRate)}
\end{equation}

An illustration can be seen below, showing how the difference is calculated along with k's impact on how much get transferred from \uppVar{b} to \uppVar{a}.
